...
| Activity | CDP discovery | Page targets only, URL query/fragment removed, bounded request/discovery timeout, and late candidate ports are not starved by earlier closed ports. | `automated`; disposable live Chromium CDP evidenced; real browsers/UIA `manual_pending` |
...
| Data | Export/import/controls | Tray/App controls dispatch through the Tk owner thread for metadata inventory, allowlisted export, clear logs, clear personal data, age-based log retention, and diagnostic-bundle preview. Export defaults to logs/metadata; sensitive categories require explicit selection and are recorded in a SHA-256 manifest. Explicit CLI recovery imports only allowlisted user-data categories after manifest/hash, settings, and SQLite validation; runtime control metadata cannot be restored. Destructive actions use allowlists and metadata-only audit files. Diagnostic bundles exclude settings, tasks, camera files, and exports. | `automated`; live dialog interaction and target-user recovery `manual_pending` |
