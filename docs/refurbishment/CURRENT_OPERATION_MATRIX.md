...
| Data | Export/controls | Tray/App controls dispatch through the Tk owner thread for metadata inventory, allowlisted export, clear logs, clear personal data, and age-based log retention. Export defaults to logs/metadata; sensitive categories require explicit selection and are recorded in a SHA-256 manifest. Destructive actions use allowlists and metadata-only audit files. | `automated`; live dialog interaction `manual_pending` |
