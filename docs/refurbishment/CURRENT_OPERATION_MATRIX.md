...
| Data | Export/controls | Tray/App controls dispatch through the Tk owner thread for metadata inventory, allowlisted export, clear logs, clear personal data, age-based log retention, and diagnostic-bundle preview. Export defaults to logs/metadata; sensitive categories require explicit selection and are recorded in a SHA-256 manifest. Destructive actions use allowlists and metadata-only audit files. Diagnostic bundles exclude settings, tasks, camera files, and exports. | `automated`; live dialog interaction `manual_pending` |
