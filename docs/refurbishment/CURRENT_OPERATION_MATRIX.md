...
| Launch | Supervised start | `focuscheck_supervisor.py` owns heartbeat, generation, PID-bound stop, stale detection, and bounded restart behavior. | `automated`; live restart `manual_pending` |
| Startup | Run-key registration | Per-user startup points at the supervised entry point; inspection distinguishes absent, valid, stale, malformed, and error states. | `automated`; registry `manual_pending` |
...
| Data | Tasks | SQLite schema journal, one active task, UTC timestamp boundary, backup/restore. | `automated`; full UI flow `manual_pending` |
...
| Native | Overlay/tray/session hooks | Pointer-sized declarations and idempotent overlay resource release. | source/unit; live Windows `manual_pending` |
