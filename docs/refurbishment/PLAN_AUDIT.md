...
| 27 Automated test expansion | Partial | 162 unittest cases, self-tests, an isolated native overlay self-test, a bounded packaged supervisor readiness/reaping self-test, a bounded performance soak, repository/process guards, frozen-entrypoint/startup and disposable install/upgrade/uninstall/package lifecycle tests, privacy/logging, structured settings-save results, settings/path-migration, runtime-state, manual-pause-preserving snooze, tray task-dialog owner-thread dispatch, tray exit lifecycle, activity-confidence, website-domain coercion, heartbeat write-failure, PID-reuse lock, sequence receipt-time freshness, stable-ready backoff, and wrap-safe Windows idle-guard matrices, and a native cleanup regression exist. Property tests, broader integration breadth, withdrawn-root Tk tests, native live tests beyond the overlay smoke path, and mutation testing are absent. |
