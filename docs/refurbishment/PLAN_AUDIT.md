...
| 6 Runtime state coordinator | Partial | `RuntimeStateCoordinator` owns transactional pause/snooze mutation, expiry-aware state, refreshed settings adoption, exclusive prompt/intervention/shutdown leases, clock-injected prompt eligibility, and an App-wired metadata-only transition journal. Full guard synchronization and crash/restart recovery evidence remain absent. |
