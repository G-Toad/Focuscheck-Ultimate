...
| 8 Supervisor and heartbeat | Partial | Versioned generation/readiness/sequence heartbeat validation, sequence receipt-time freshness with explicit cadence, PID-bound JSON stop requests, nonce/supervisor/generation/process-start/freshness validation, atomic stop writes, frozen inner-PID handling, process-start-token lock ownership, stale checks, force-start correction, circuit breaker, and lifecycle tests now exist. Acknowledgement timeout, sleep-gap handling, and the complete failure matrix are not proven. |
...
| 27 Automated test expansion | Partial | 155 unittest cases, self-tests, an isolated native overlay self-test, a bounded packaged supervisor readiness/reaping self-test, a bounded performance soak, repository/process guards, frozen-entrypoint/startup and disposable install/upgrade/uninstall/package lifecycle tests, privacy/logging, settings/path-migration, runtime-state, activity-confidence, website-domain coercion, PID-reuse lock, and sequence receipt-time freshness matrices, and a native cleanup regression exist. Property tests, broader integration breadth, withdrawn-root Tk tests, native live tests beyond the overlay smoke path, and mutation testing are absent. |
