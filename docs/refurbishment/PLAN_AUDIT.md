...
| 8 Supervisor and heartbeat | Partial | Versioned generation/readiness/sequence heartbeat validation, sequence receipt-time freshness with explicit cadence, stable-ready backoff reset, atomic heartbeat publication with throttled write-failure diagnostics, PID-bound JSON stop requests, nonce/supervisor/generation/process-start/freshness validation, atomic stop writes, frozen inner-PID handling, process-start-token lock ownership, stale checks, force-start correction, circuit breaker, and lifecycle tests now exist. Acknowledgement timeout, sleep-gap handling, and the complete failure matrix are not proven. |
...
| 27 Automated test expansion | Partial | 164 unittest cases, self-tests, an isolated native overlay self-test, a bounded packaged supervisor readiness/reaping self-test, a bounded performance soak, repository/process guards, frozen-entrypoint/startup and disposable install/upgrade/uninstall/package lifecycle tests, privacy/logging, structured settings-save results, settings/path-migration including task/log conflict migration, runtime-state, mainloop exception preservation, manual-pause-preserving snooze, tray task-dialog owner-thread dispatch, tray exit lifecycle, activity-confidence, website-domain coercion, heartbeat write-failure, PID-reuse lock, sequence receipt-time freshness, stable-ready backoff, and wrap-safe Windows idle-guard matrices, and a native cleanup regression exist. Property tests, broader integration breadth, withdrawn-root Tk tests, native live tests beyond the overlay smoke path, and mutation testing are absent. |
