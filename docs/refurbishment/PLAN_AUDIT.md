...
| 6 Runtime state coordinator | Partial | `RuntimeStateCoordinator` owns transactional pause/snooze mutation, manual-pause-preserving snooze expiry, refreshed settings adoption, exclusive prompt/intervention/shutdown leases, clock-injected prompt eligibility, prompt denial during effective pause, website-check suppression through the coordinator, and an App-wired metadata-only transition journal. Guard health now publishes bounded native failure metadata, but full guard synchronization and crash/restart recovery evidence remain absent. |
...
| 18 Settings UI | Partial | Existing UI tests, structured save-result/error handling, revision conflict result handling, and owner-thread/tray contract tests exist. Schema-generated controls, complete visible-control round trips, runtime-consumer proof, and manual UI evidence are absent. |
...
| 27 Automated test expansion | Partial | 171 unittest cases, self-tests, an isolated native overlay self-test, a bounded packaged supervisor readiness/reaping self-test, a bounded performance soak, repository/process guards, frozen-entrypoint/startup and disposable install/upgrade/uninstall/package lifecycle tests, privacy/logging, structured settings-save results, settings/path-migration including task/log conflict migration, runtime-state, lifecycle phase transitions, mainloop exception preservation, manual-pause-preserving snooze, tray task-dialog owner-thread dispatch, tray exit lifecycle, activity-confidence, website-domain coercion, heartbeat write-failure, PID-reuse lock, sequence receipt-time freshness, stable-ready backoff, cancellation-aware supervisor waits, durable generation-bound stop acknowledgement, pause-guard native failure diagnostics, startup legacy/duplicate inspection and repair, explicit single-instance mutex handle ownership, and wrap-safe Windows idle-guard matrices, and a native cleanup regression exist. Property tests, broader integration breadth, withdrawn-root Tk tests, native live tests beyond the overlay smoke path, and mutation testing are absent. |
...
Latest verification rerun: `171` unittest cases pass, including lifecycle phase transition/failure coverage, cancellation-aware supervisor waits, durable generation-bound stop acknowledgement, pause-guard native failure diagnostics, startup legacy/duplicate inspection and repair, and single-instance mutex handle release coverage.
