...
| Anti-habit hold | When hold enforcement is enabled, mouse release must satisfy the configured duration and keyboard activation must not bypass it. | `tests/test_v2_flows.py::test_v1_anti_habit_hold_uses_composed_monotonic_clock`; `test_v1_keyboard_activation_cannot_bypass_hold_requirement`. | automated; interactive validation manual_pending |
