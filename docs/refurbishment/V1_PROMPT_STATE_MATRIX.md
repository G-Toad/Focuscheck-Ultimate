...
| Focus detail enabled/disabled | Disabled detail skips its child dialog; enabled detail validates and returns to the owning prompt path; initial-focus work is owned and cancelled with dialog destruction. | Dialog callback/failure-isolation tests; settings-consumer coverage; `test_focus_and_waste_dialogs_own_initial_focus_timer`. | partial; enabled interactive flow manual_pending |
