...
| Studying response | A valid studying choice reaches one completion callback and one focus-log attempt. | `test_v1_prompt_notifies_owner_after_studying_choice`; prompt finalization tests. | automated; full detail flow manual_pending |
| Wasting-time response | A valid wasting choice reaches one completion callback and one waste-log attempt. | QA scenario and CSV logging tests. | automated; full detail flow manual_pending |
...
| Focus detail enabled/disabled | Disabled detail skips its child dialog; enabled detail validates and returns to the owning prompt path. | Dialog callback/failure-isolation tests; settings-consumer coverage. | partial; enabled interactive flow manual_pending |
| Waste detail enabled/disabled | Disabled detail skips its child dialog; enabled detail validates and returns to the owning prompt path. | Dialog callback/failure-isolation tests; settings-consumer coverage. | partial; enabled interactive flow manual_pending |
| Challenge enabled/disabled | Challenge cancellation or validation failure does not claim a successful prompt completion. | Phrase/challenge timer and callback cleanup tests. | partial; complete challenge matrix manual_pending |
