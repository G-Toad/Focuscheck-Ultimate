...
5. App treats `query_end_session` as preparation only; on committed shutdown it enters `stopping`, atomically writes a nonce- and generation-bound intentional-stop request, and flushes owned resources before supervised exit.
