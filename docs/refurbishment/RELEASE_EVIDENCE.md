...
The current automated baseline is `188` tests; the detailed first bullet is retained as historical baseline context.
...
- Current verification rerun: `188` unittest cases pass; it additionally covers centralized guard refresh/duplicate-transition suppression, lifecycle phase transition/failure, Windows shutdown query/end-session separation, structured event redaction/rotation, retention clock/symlink/audit coverage, settings UI/schema drift and visible runtime-consumer proof, cancellation-aware supervisor waits, durable generation-bound stop acknowledgement, pause-guard native failure diagnostics, startup legacy/duplicate inspection and repair, single-instance mutex handle release, activity/window-enumeration ctypes signatures, timezone/DST analytics boundaries, legacy task migration fixture/normalization, and integrity-checked migration backup/corruption rejection.
