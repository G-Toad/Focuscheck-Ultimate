...
- Latest verified source checkpoint: `cd42777`; 567 unittest cases and all 20 bounded verification stages pass with zero process leaks. Intensification child-window positioning now declares its `SetWindowPos` ctypes contract before native calls. Manual Windows and release evidence remains pending.
