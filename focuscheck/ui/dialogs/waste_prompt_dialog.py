"""
Dialog shown when user clicks 'Wasting time' to capture waste details.
"""
...
class WastePromptDialog(tk.Toplevel):
    ...
    def __init__(self, master, ask_what=True, ask_consequences=True, on_submit=None, on_cancel=None, auto_focus=True, require_all_fields=False, settings=None, monotonic_clock=None):
        ...
        self.validation_enabled = bool(self.settings.get("wasting_prompt_validation_enabled", True))
        ...
        if self.ask_what:
            ...
            if self.active_challenge:
                what_prompt = self.active_challenge["question"]
            else:
                what_prompt = "What are you wasting time on?"

            ttk.Label(self, text=what_prompt, wraplength=450).grid(row=row, column=0, columnspan=2, sticky="w", **pad)
            row += 1
            ...
            what_entry.grid(row=row, column=0, columnspan=2, sticky="we", **pad)
    ...
    def _init_spam_detector(self):
        ...
        logger = get_logger()
        if not self.validation_enabled or not is_spam_detection_enabled(self.settings):
    ...
    def _init_challenge_system(self):
        ...
        # Select a challenge for "wasting" context
        self.active_challenge = self.challenge_system.get_challenge(context="wasting")
