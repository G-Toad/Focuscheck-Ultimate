...
class App:
    ...
    def _diagnostic_status_snapshot(self) -> dict:
        ...
        if service is None:
            ...
            service = HealthSnapshotService(self, app_name=APP_NAME, app_version=APP_VERSION)
            self._health_snapshot_service = service
    ...
    def _write_heartbeat(self):
        """Publish the structured heartbeat through the protocol service."""
        service = getattr(self, "_heartbeat_service", None)
        if service is None:
            from .runtime.heartbeat import HeartbeatService
            factory = getattr(getattr(self, "_dependencies", None), "heartbeat_service_factory", None)
            service = factory(self) if callable(factory) else HeartbeatService(
                self,
                heartbeat_interval_ms=FILE_HEARTBEAT_INTERVAL_SECONDS,
                default_path=HEARTBEAT_PATH,
                logger_factory=get_logger,
            )
